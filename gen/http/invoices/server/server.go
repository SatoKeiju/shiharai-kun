...
// New instantiates HTTP handlers for all the invoices service endpoints using
...
func (s *Server) Service() string { return "invoices" }
...
func (s *Server) MethodNames() []string { return invoices.MethodNames[:] }
...
// MountCreateHandler configures the mux to serve the "invoices" service
// "create" endpoint.
func MountCreateHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/api/invoices", f)
}

// NewCreateHandler creates a HTTP handler which loads the HTTP request and
// calls the "invoices" service "create" endpoint.
